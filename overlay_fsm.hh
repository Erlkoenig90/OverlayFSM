...
namespace Helper {
	...
	template<typename Type, bool = std::is_literal_type<Type>::value>
	...
	union MultiUnion<First, Rest...> {
		...
		template <typename... Args>
		constexpr First* construct (UnionTag<First>, Args&&... args) {
		...
		constexpr First& get (UnionTag<First>) {
			return reinterpret_cast<First&> (first.storage);
		}
...
class OverlayFSM {
	public:
		...
		static_assert ((std::is_base_of<StateBase, States>::value && ...), "All states must be derived from the StateBase class!");
		static_assert ((!std::is_abstract<States>::value && ...), "State classes may not contain pure virtual member functions!");
	private:
	...
	public:
		...
		my_always_inline constexpr OverlayFSM (S*, Args&&... args) : m_memory {Helper::UnionTag<S> {}, std::forward<Args> (args)...}, m_current { &m_memory.get (Helper::UnionTag<S> {}) } {  }

		/// Ruft den Destruktor des aktuellen Zustands, auf dem Typ der Basisklasse, auf.
		my_always_inline ~OverlayFSM () { current ().~StateBase (); }

		/// Gibt eine Referenz auf den aktuellen Zustand zurück, als Typ der Basisklasse.
		my_always_inline StateBase& current () { return *m_current; }
		/// Gibt eine konstante Referenz auf den aktuellen Zustand zurück, als Typ der Basisklasse.
		my_always_inline const StateBase& current () const { return *m_current; }
		 ...
		 */
		template <typename S, typename... Args>
		my_always_inline S& go (Args&&... args) {
			// Prüfe, ob der Zustand auch in den Klassem-Template-Parametern (States...) enthalten ist
			static_assert ((std::is_same<States, S>::value || ...), "Nonexisting state passed to OverlayFSM::go");
			
			// Destruktor aufrufen
			current ().~StateBase ();
			
			return *m_memory.construct (Helper::UnionTag<S> {}, std::forward<Args> (args)...);
		}
};
