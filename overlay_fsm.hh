...
class OverlayFSM : public Helper::MixinDestructor<OverlayFSM<StateBase_, States...>, std::is_trivially_destructible<StateBase_>::value> {
	...
	public:
		...
		always_inline constexpr OverlayFSM (S*, Args&&... args) : m_memory {Helper::UnionTag<S> {}, std::forward<Args> (args)...}, m_current { &m_memory.get (Helper::UnionTag<S> {}) } {  }

		/// Gibt eine Referenz auf den aktuellen Zustand zurück, als Typ der Basisklasse.
		always_inline StateBase& current () { return *m_current; }
		/// Gibt eine konstante Referenz auf den aktuellen Zustand zurück, als Typ der Basisklasse.
		always_inline const StateBase& current () const { return *m_current; }
